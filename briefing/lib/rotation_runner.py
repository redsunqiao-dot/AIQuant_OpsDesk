...
def list_sectors(level: int = 2, conn=None) -> List[Dict]:
...
def get_sector_member_codes(sector_name: str, level: int = 2, conn=None) -> List[str]:
    """取板块当前成分股代码列表"""
    field = "sector_1" if level == 1 else "sector_2"
    rows = execute_query(
...
def _rows_to_sector_df(rows: List[Dict]) -> pd.DataFrame:
    if not rows:
        return pd.DataFrame()
...
def load_sector_index(sector_name: str, level: int = 2,
                       end_date: Optional[str] = None, conn=None) -> pd.DataFrame:
    ...
    """
    return _rows_to_sector_df(execute_query(sql, params, conn=conn))


def load_all_sectors(level: int = 2, end_date: Optional[str] = None,
                      min_days: int = 70) -> Dict[str, pd.DataFrame]:
    """一次查出该级别全部板块指数，避免每板块开关连接触发 WinError 10048。"""
    ...
    sql = f"""
        ...
        FROM trade_sector_daily
        WHERE {' AND '.join(conditions)}
        ORDER BY sector_name ASC, trade_date ASC
    """
    conn = get_connection()
    try:
        ...
        for r in rows:
            by_name.setdefault(r["sector_name"], []).append(r)

        result = {}
        for name, sector_rows in by_name.items():
            cleaned = [{k: v for k, v in row.items() if k != "sector_name"}
                       for row in sector_rows]
            df = _rows_to_sector_df(cleaned)
