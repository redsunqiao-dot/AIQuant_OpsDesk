...
Charles 加载路径固定为作战台根下 vendor/charles/charles-nanobot/agent.py ,
...
# Charles 加载路径（优先 vendor/charles，兼容旧 third_party/charles_bundle）
_ROOT = Path(__file__).resolve().parents[1]
_CHARLES_CANDIDATES = (
    _ROOT / "vendor" / "charles" / "charles-nanobot" / "agent.py",
    _ROOT / "third_party" / "charles_bundle" / "charles-nanobot" / "agent.py",
)
_CHARLES_AGENT_PY = next((p for p in _CHARLES_CANDIDATES if p.is_file()), _CHARLES_CANDIDATES[0])


def _resolve_charles_path() -> Path | None:
    """返回 agent.py 路径；文件不存在则为 None."""
    for p in _CHARLES_CANDIDATES:
        if p.is_file():
            return p
    return None
...
def _ensure_charles_bot():
    ...
    if p is None:
        msg = ("找不到 nanobot Charles: "
               f"{_CHARLES_AGENT_PY} （请确认 vendor/charles 下已有 charles-nanobot 与 nanobot-main）")
        _CHARLES_CACHE["load_err"] = msg
        raise RuntimeError(msg)
...
def build_tab():
    with gr.Row(equal_height=False):
        ...
        with gr.Column(scale=4):
            # Gradio 6: 默认即 messages 格式；无 type / show_copy_button
            chatbot = gr.Chatbot(
                label=None,
                height=600,
                show_label=False,
                ...
                placeholder="跟 Charles 聊聊吧 — 右侧有推荐对话，点一下就能开聊",
            )
