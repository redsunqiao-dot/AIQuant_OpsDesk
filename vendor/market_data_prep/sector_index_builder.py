...
from db_config import execute_query, execute_many, execute_update, get_connection
...
def get_all_trade_dates(conn=None) -> List[date]:
...
def get_prev_close_idx(sector_name: str, sector_level: int,
                        prev_trade_date: date, conn=None) -> float:
...
def load_sector_members_map(level: int = 2, conn=None) -> Dict[str, List[str]]:
    """一次查出全部板块成分股，避免每板块单独开连接。"""
    field = "sector_1" if level == 1 else "sector_2"
    rows = execute_query(
        f"SELECT stock_code, {field} AS name FROM trade_stock_status "
        f"WHERE {field} IS NOT NULL ORDER BY stock_code",
        conn=conn)
    out: Dict[str, List[str]] = {}
    for r in rows:
        out.setdefault(r["name"], []).append(r["stock_code"])
    return out


def fetch_member_kline_panel(member_codes: List[str],
                              trade_dates: List[date],
                              conn=None) -> Dict[Tuple[str, date], Dict]:
    """
...
def save_sector_rows(rows: List[Dict], conn=None) -> int:
    ...
    return execute_many(sql, rows_tuples, conn=conn)
...
def rebuild_one_sector(sector_name: str, sector_level: int,
    ...
    panel = fetch_member_kline_panel(members, sorted(fetch_dates), conn=conn)
    ...
    for d in target_dates:
        ...
        if prev_close_idx is None:
            prev_close_idx = (
                get_prev_close_idx(sector_name, sector_level, prev_d, conn=conn)
                if prev_d else BASE_INDEX
            )
    ...
    return save_sector_rows(rows, conn=conn)
...
def rebuild_all_sectors(level: int = 2,
    ...
    conn = get_connection()
    try:
        all_dates = get_all_trade_dates(conn=conn)
        if not all_dates:
        ...
        members_map = load_sector_members_map(level=level, conn=conn)
        sectors = sorted(members_map.keys()) or list_sectors_from_db(level=level)
        print(f"[INDEX] 待重算板块: {len(sectors)}, 日期范围: "
              f"{target_dates[0]} ~ {target_dates[-1]} ({len(target_dates)} 个交易日)\n")

        total = 0
        for i, sector in enumerate(sectors, 1):
            n = rebuild_one_sector(
                sector, level, target_dates, all_dates,
                members=members_map.get(sector),
                conn=conn,
            )
            total += n
