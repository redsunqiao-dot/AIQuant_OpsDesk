...
def init_schema():
    ...
    # 先按分号切开，再去掉纯注释行；不能用 startswith('--') 过滤整段，
    # 否则「注释开头 + CREATE TABLE」会被误跳过。
    raw_statements = [s.strip() for s in ddl_text.split(";") if s.strip()]
    print(f"\n{'='*70}")
    ...
    for stmt in raw_statements:
        ...
        created += 1
        print(f"  [OK] 执行 DDL #{created}")
    print(f"[OK] 表结构创建完成 ({created} 条 DDL)\n")
