...
def build_industry_map(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    数据来源: public_market.fetch_industry_classification + get_hs300_codes
...
def _load_real_positions() -> List[Dict[str, Any]]:
    """读 real 当前持仓 (file_bridge 账户快照 / miniQMT 只读)

    字段统一适配为: code / cost / cur_price / volume / market_value / name
    cur_price = market_value / volume (反推)
    ...
    def _adapt(raw: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        out = []
        for p in raw:
    ...
    if broker_mode() == "file_bridge":
        snap = read_account_snapshot(max_age_sec=120.0)
        raw = list(snap.get("positions") or [])
        if not raw and snap.get("error"):
            raise RuntimeError(snap.get("error"))
        return _adapt(raw)
    ...
    trader.connect()
    return _adapt(list(trader.query_positions() or []))
