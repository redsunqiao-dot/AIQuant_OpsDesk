...
设计:
    ...
    - 持仓: 优先同步大 QMT 账户快照 (file_bridge); 没有快照时用 config/mock_positions.yaml
...
def merge_watch_codes(ui_codes: Optional[List[str]] = None) -> List[str]:
    """
    合并监控代码 (去重, 顺序: 页面输入 -> 自选池 -> 策略路由里的代码)。

    模拟持仓不并入。晨会入选会整池替换 watch_pool 与 per_stock。
...
def _lookup_stock_names(codes: List[str]) -> dict:
    """批量查中文简称: trade_stock_status (晨会同一库)。"""
    out = {}
    ...
    try:
        ...
        print(f"[WARN] 查股票名称失败: {e}", flush=True)
    return out


def _normalize_position_rows(raw_positions: List[dict]) -> List[dict]:
    """把 {code,name,volume,cost} 列表补齐 cur_price / 市值 / 盈亏。

    快照通常没有中文名, 缺 name 时用 trade_stock_status 补全。
    """
    codes = [p.get("code", "") for p in raw_positions if p.get("code")]
    close_map = _latest_close_map(codes)
    need_name = [c for c, p in zip(
        [p.get("code", "") for p in raw_positions],
        raw_positions,
    ) if c and not str(p.get("name") or "").strip()]
    name_map = _lookup_stock_names(need_name) if need_name else {}
    ...
    for p in raw_positions:
        ...
        if not code or volume <= 0:
            continue
        name = str(p.get("name") or "").strip() or name_map.get(code, "")
        cur_price = float(close_map.get(code, cost if cost > 0 else 0))
        if cur_price <= 0:
            cur_price = cost
        market_value = volume * cur_price
        pnl = (cur_price - cost) * volume if cost > 0 else 0.0
...
def build_positions_from_config() -> List[dict]:
    ...
    return _normalize_position_rows(raw_positions)


def try_build_positions_from_snapshot(
    max_age_sec: float = 86400.0,
) -> Optional[Tuple[List[dict], float, str]]:
    """优先从大 QMT 文件桥账户快照构建模拟盘初始持仓。

    即使快照略过期, 只要文件里有资金/持仓数据就采用 (教学场景停策略后仍希望对上实盘)。
    返回 (positions, capital, source_desc); 不可用时返回 None。
    """
    try:
        from live_trading.qmt_file_bridge import read_account_snapshot
    except Exception as e:
        print(f"[WARN] 读账户快照模块失败: {e}", flush=True)
        return None

    snap = read_account_snapshot(max_age_sec=float(max_age_sec))
    age = float(snap.get("age_sec") or -1)
    # 文件不存在时 age_sec=-1 且无持仓
    if age < 0 and not (snap.get("positions") or snap.get("asset")):
        return None

    asset = snap.get("asset") or {}
    capital = float(asset.get("total_asset") or asset.get("cash") or 0)
    raw_rows = []
    for p in list(snap.get("positions") or []):
        if not isinstance(p, dict):
            continue
        code = str(p.get("stock_code") or p.get("code") or "").strip()
        volume = int(p.get("volume") or 0)
        cost = float(p.get("open_price") or p.get("cost") or 0)
        if not code or volume <= 0:
            continue
        raw_rows.append({
            "code": code,
    ...
    # 允许空仓: 只要资金有效, 就用快照 (对齐实盘空仓状态)
    if capital <= 0 and not raw_rows:
        return None

    if capital <= 0:
        # 无总资产字段时, 用持仓市值兜底, 避免 capital=0 把仓位比例算炸
        positions_tmp = _normalize_position_rows(raw_rows)
        capital = sum(float(x.get("market_value") or 0) for x in positions_tmp) or 1_000_000.0
        positions = positions_tmp
    else:
        positions = _normalize_position_rows(raw_rows)

    acct = str(snap.get("account_id") or "")
    stale = "" if snap.get("ok") else f", 快照年龄 {age:.0f}s"
    source = f"file_bridge 账户快照{(' ' + acct) if acct else ''}{stale}"
    return positions, capital, source


def build_initial_positions() -> Tuple[List[dict], float, str]:
    """模拟盘启动/重置用的初始持仓。

    优先大 QMT 账户快照; 没有可用快照时回退 mock_positions.yaml。
    返回 (positions, capital, source_desc)。
    """
    got = try_build_positions_from_snapshot()
    if got is not None:
        return got
    positions = build_positions_from_config()
    cfg = load_mock_config()
    capital = float(cfg.get("capital", 1_000_000))
    return positions, capital, "config/mock_positions.yaml"
...
class LiveSimRunner:
    ...
    def start(self, watch_stocks: List[str], cycle_seconds: int = 60,
        ...
        Args:
            ...
            init_positions: 是否初始化持仓 (优先账户快照, 否则 mock yaml)
        """
        ...
        # 初始仓 / 资金: 优先同步实盘快照
        init_pos, capital, pos_source = build_initial_positions()
        ...
        print(f"[INFO] 模拟盘初始持仓来源: {pos_source}, 共 {len(init_pos if init_positions else s.get('positions', []))} 只, capital={capital:.2f}", flush=True)
    ...
    def bind_matched_strategies(self, codes: List[str]) -> str:
        """按近一年和再往前一年的日线回测，给每只股票单独绑定策略，并整表替换 per_stock。"""
        from concurrent.futures import ThreadPoolExecutor, as_completed
        from datetime import date, timedelta
        from lib.backtest_engine import match_strategy_for_stock
        ...
        def _match_one(code: str) -> tuple:
            print(f"[match] {code} 近端 {start} ~ {end}，并再往前同样长度", flush=True)
            try:
                picked = match_strategy_for_stock(code, start, end)
            except Exception as e:
                return code, None, f"{type(e).__name__}: {e}"
            return code, picked, None

        # 多票并行匹配; 单票内部 score_strategies 已并行跑候选
        workers = max(1, min(3, len(codes)))
        results = []
        with ThreadPoolExecutor(max_workers=workers) as pool:
            futs = {pool.submit(_match_one, c): c for c in codes}
            for fut in as_completed(futs):
                results.append(fut.result())

        # 按入选顺序输出
        by_code = {r[0]: r for r in results}
        for code in codes:
            code, picked, err = by_code.get(code, (code, None, "无结果"))
            if err:
                unmatched.append(code)
                lines.append(f"{code} 匹配异常:{err}")
                continue
            if not picked:
                unmatched.append(code)
                lines.append(f"{code} 未匹配")
                continue
            per[code] = picked["strategy"]
            lines.append(
                f"{code}->{picked['strategy']}"
                f"(较差卡玛{picked['calmar']:.2f}"
                f" 近端年化{picked['annual_return']:+.1%}"
                f" 前段年化{picked['prior_annual']:+.1%})"
            )
        cfg = load_strategy_config()
        default = cfg.get("default") or "macd_1d"
        msg = self.apply_strategy_config(default, per)
        extra = ""
        if unmatched:
            extra = f" 未单独绑定、走默认 {default}: {', '.join(unmatched)}。"
        return f"{'; '.join(lines)}。{extra}{msg}"

    def adopt_morning_watch(self, codes: List[str], strategy: Optional[str] = None) -> str:
        """用晨会入选整池替换监控名单。

        未指定 strategy 时，按近一年和再往前一年的日线回测给每只票匹配策略。
        指定 strategy 时仍整池绑同一个策略。
        旧 watch_pool 与旧 per_stock 全部清掉，不追加。
        入选为空时不改动，避免误清空。
        """
        cleaned: List[str] = []
        seen = set()
        for raw in codes or []:
            ...
            cleaned.append(code)
        if not cleaned:
            return "[SKIP] 晨会入选为空, 监控池未改"
        save_watch_pool(cleaned)
        if strategy:
            msg = self.apply_strategy_config(strategy, {c: strategy for c in cleaned})
            bound = f"策略={strategy}"
        else:
            msg = self.bind_matched_strategies(cleaned)
            bound = "已按近一年和前一年日线回测自动匹配"
        self._last_watch_stocks = list(cleaned)
        if self._loop is not None:
            self._loop.watch_stocks = list(cleaned)
            try:
                state = self._loop.state_store.load()
                state["watch_stocks"] = list(cleaned)
                self._loop.state_store.save(state)
            except Exception as e:
                print(f"[WARN] 回写 watch_stocks 失败: {e}", flush=True)
        return f"监控池已替换为晨会 {len(cleaned)} 只, {bound}。{msg}"
    ...
    def clear_history(self) -> str:
        ...
        atomic_write_text(OUTPUTS_LIVE_STATE, json.dumps(s, ensure_ascii=False, indent=2))
        return f"[OK] 已清空 {cleared} 条历史 (events/signals/orders/pnl), 持仓保留"

    # ------------------------------------------------------------------
    def reset_positions(self) -> str:
        """重新加载初始持仓覆盖 state.positions (不动 events / signals / orders)

        优先大 QMT 账户快照; 没有快照时回退 mock_positions.yaml。
        """
        from lib.paths import OUTPUTS_LIVE_STATE
        import json, os
        if not OUTPUTS_LIVE_STATE.exists():
        ...
        try:
            ...
            s = {}
        new_positions, capital, source = build_initial_positions()
        s["positions"] = new_positions
        s["capital"] = capital
        # 若模拟盘正在跑, 同步 loop 内资金, 避免仓位建议仍按旧 capital
        if self._loop is not None:
            try:
                self._loop.capital = capital
            except Exception:
                pass
        ...
        from live_trading.state_store import atomic_write_text
        atomic_write_text(OUTPUTS_LIVE_STATE, json.dumps(s, ensure_ascii=False, indent=2))
        return f"[OK] 重置持仓 -- 共 {len(new_positions)} 只, capital={capital:.0f}, 来源 {source}"
