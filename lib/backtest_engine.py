...
def score_strategies(stock_code: str,
    ...
    def _run_one(name: str) -> Dict[str, Any]:
        ...
        try:
            res = run_backtest(
                stock_code, name, start_date, end_date,
                initial_cash=initial_cash,
                commission=commission,
                position_pct=position_pct,
            )
            return {"name": name, "res": res, "error": None}
        except Exception as e:
            return {"name": name, "res": None, "error": f"{type(e).__name__}: {e}"}

    workers = max(1, min(6, len(candidates)))
    raw_results: List[Dict[str, Any]] = []
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futs = [pool.submit(_run_one, name) for name in candidates]
    ...
    for name in candidates:
        ...
        if item is None:
            failed.append({"strategy": name, "error": "无结果"})
            continue
        if item.get("error"):
            failed.append({"strategy": name, "error": item["error"]})
            continue
        res = item.get("res") or {}
        if not res.get("ok"):
...
# 日线回测能产出信号的策略。5 分钟 / 网格 / 龙头在日线回放里没有分钟 K，只会一直观望。
# 晨会自动匹配不含 ml_prob (滚动训练过慢); 回测页「推荐」可手动勾选全部策略。
DAILY_MATCH_CANDIDATES = [
    "macd_1d",
    "ma20_hold",
    "multi_factor_top",
    "rsi_reversal",
...
def _active_rows(report: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    """留下真的动过手的策略。没有卖出但净值明显变了，也算持有到期末。"""
    active: Dict[str, Dict[str, Any]] = {}
    for row in report.get("summaries") or []:
        traded = int(row.get("trades") or 0) >= 1
        moved = abs(float(row.get("annual_return") or 0)) > 0.02
        if traded or moved:
            active[row["strategy"]] = row
    return active


def match_strategy_for_stock(stock_code: str,
                             start_date: str,
                             end_date: str) -> Optional[Dict[str, Any]]:
    """给一只股票选一个日线策略。

    近端区间和再往前同样长度的一段都要跑。只保留两段都动过手的策略，
    按较差那段的卡玛、夏普排序，不按年化收益最高取。
    """
    from datetime import datetime, timedelta
    recent_start = datetime.strptime(start_date, "%Y-%m-%d")
    ...
    recent = _active_rows(score_strategies(
        stock_code, start_date, end_date,
        candidates=list(DAILY_MATCH_CANDIDATES),
    ))
    prior = _active_rows(score_strategies(
        stock_code, prior_start_s, prior_end_s,
        candidates=list(DAILY_MATCH_CANDIDATES),
    ))
    common = [name for name in recent if name in prior]
    if not common:
        return None
    ...
    def _rank_key(name: str):
        a = recent[name]
        b = prior[name]
        both_pos = float(a.get("sharpe") or 0) > 0 and float(b.get("sharpe") or 0) > 0
        worst_calmar = min(float(a.get("calmar") or 0), float(b.get("calmar") or 0))
        worst_sharpe = min(float(a.get("sharpe") or 0), float(b.get("sharpe") or 0))
        return (both_pos, worst_calmar, worst_sharpe)

    best_name = max(common, key=_rank_key)
    a = recent[best_name]
    b = prior[best_name]
    return {
        "strategy":         best_name,
        "label":            a.get("label") or best_name,
        "calmar":           min(float(a.get("calmar") or 0), float(b.get("calmar") or 0)),
        "sharpe":           min(float(a.get("sharpe") or 0), float(b.get("sharpe") or 0)),
        "annual_return":    float(a.get("annual_return") or 0),
        "prior_annual":     float(b.get("annual_return") or 0),
        "recent_calmar":    float(a.get("calmar") or 0),
        "prior_calmar":     float(b.get("calmar") or 0),
        "max_drawdown":     float(a.get("max_drawdown") or 0),
        "trades":           int(a.get("trades") or 0),
        "start_date":       start_date,
        "end_date":         end_date,
        "prior_start":      prior_start_s,
