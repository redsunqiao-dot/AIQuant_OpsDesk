...
def _query_real_total_asset() -> dict:
    """读实盘总资产: file_bridge 走账户快照; 否则走 miniQMT 只读.

    Returns:
        {"ok": bool, "message": str, "total_asset": float|None, "source": str}
    """
    from live_trading.qmt_file_bridge import broker_mode, read_account_snapshot

    if broker_mode() == "file_bridge":
        # 日频 NAV 记录允许稍旧; 无资产字段才算失败
        ...
        if total_asset <= 0:
            return {
                "ok": False,
                "message": snap.get("error") or "账户快照无 total_asset, 请确认大 QMT 文件桥策略在跑",
                "total_asset": None,
                "source": "file_bridge",
            }
        stale_note = f" (快照 {snap.get('age_sec')}s 前)" if snap.get("error") else ""
        return {
            "ok": True,
            "message": f"file_bridge 账户快照{stale_note}",
    ...
    if not qmt_path or not account_id:
        return {"ok": False, "message": "QMT_PATH / ACCOUNT_ID 未配置 (.env)",
                "total_asset": None, "source": "miniqmt"}
    try:
        setup_sys_path()
        ...
        return {"ok": False,
                "message": f"miniQMT 连接/查询失败: {type(e).__name__}: {e}",
                "total_asset": None, "source": "miniqmt"}
    total_asset = float(asset.get("total_asset") or 0)
    if total_asset <= 0:
        return {"ok": False, "message": f"total_asset = {total_asset}, 实盘账户为空或未连接",
                "total_asset": None, "source": "miniqmt"}
    return {"ok": True, "message": "miniQMT 只读", "total_asset": total_asset,
            "source": "miniqmt"}
...
def _record_real_pnl_snapshot(force: bool = False) -> dict:
    ...
    Returns:
        {"ok": bool, "message": str, "baseline": float|None,
         ...
         "data_points": int}
    """
    q = _query_real_total_asset()
    if not q.get("ok"):
        return {"ok": False, "message": q.get("message") or "读实盘总资产失败",
                "baseline": None, "today_total": None, "today_pct": None, "data_points": 0}

    total_asset = float(q["total_asset"])
    src = q.get("source") or ""
    ...
    if today_records and not force:
        ...
        msg = (f"已记录 {today} 快照[{src}]: total_asset={total_asset:.2f}, "
...
def lifecycle_sim_eval(payload: dict = Body({})):
    ...
    if source == "sim":
        ...
        # real: 评估前顺手写今天一条 (同日已有则跳过), 与 /live/real 自动累积一致
        try:
            _record_real_pnl_snapshot(force=False)
        except Exception:
            pass
    ...
    if len(pnl_history) < 2:
        if source == "real":
            ...
            return {
                "ok": False,
                "message": (f"实盘 NAV 历史只有 {n} 个交易日, 至少需要 2 天才能算 KPI "
                            f"(建议累积 ≥20 天再评估升降级). "
                            f"同日重复点「记录实盘快照」不会新增; 请隔日再记, "
                            f"或打开 /live/real 让系统每个交易日自动写一条."),
            }
...
@router.post("/qs_report")
@router.post("/qs_report/")
def review_qs_report(payload: dict = Body(None)):
    """从模拟盘/实盘净值历史生成 QuantStats HTML.

    body: {source: "sim"|"real", title?: str}
    """
    payload = payload or {}
    source = str(payload.get("source") or "sim").strip().lower()
    title = str(payload.get("title") or f"{source} 净值 QuantStats")

    navs = []
    if source == "sim":
        if not OUTPUTS_LIVE_STATE.exists():
            return {"ok": False, "message": "live_state.json 不存在"}
        state = json.loads(OUTPUTS_LIVE_STATE.read_text(encoding="utf-8"))
        hist = state.get("pnl_history") or []
        # pnl_history 常为 {ts, pnl_pct}; 转近似净值 = 1+pct
        for h in hist:
            d = h.get("ts") or h.get("date") or h.get("time")
            if "nav" in h:
                navs.append({"date": d, "nav": float(h["nav"])})
            elif "total_asset" in h:
                navs.append({"date": d, "nav": float(h["total_asset"])})
            elif "pnl_pct" in h:
                navs.append({"date": d, "nav": 1.0 + float(h["pnl_pct"])})
    elif source == "real":
        data = _load_real_pnl_history()
        for h in data.get("history") or []:
            d = h.get("ts") or h.get("date")
            if "total_asset" in h:
                navs.append({"date": d, "nav": float(h["total_asset"])})
            elif "cum_pct" in h:
                base = float(data.get("baseline_total_asset") or 1.0)
                navs.append({"date": d, "nav": base * (1.0 + float(h["cum_pct"]))})
    else:
        return {"ok": False, "message": "source 应为 sim 或 real"}

    try:
        from lib.qs_report import generate_qs_html_from_navs
        info = generate_qs_html_from_navs(
            navs, title=title, out_name=f"review_{source}_qs.html",
        )
        return {"ok": True, "report": info, "source": source, "points": len(navs)}
    except Exception as e:
        return {"ok": False, "message": f"{type(e).__name__}: {e}", "points": len(navs)}
