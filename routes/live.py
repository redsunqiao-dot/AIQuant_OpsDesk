...
POST /api/live/adopt_radar           -- 用 04 TALib 最新精选池替换监控池
...
def _load_state() -> dict:
    ...
    try:
        ...
        return json.loads(read_text_shared(OUTPUTS_LIVE_STATE))
    except Exception:
        return _empty_state()
...
def _save_state(state: dict):
    ...
    from live_trading.state_store import atomic_write_text
    atomic_write_text(
        OUTPUTS_LIVE_STATE,
...
def sim_reset_positions(payload: dict = Body({})):
    """重置模拟盘持仓: 优先账户快照, 否则 mock_positions.yaml"""
...
@router.post("/adopt_radar")
def adopt_radar(payload: Optional[Dict[str, Any]] = Body(None)):
    """用 04 TALib 最新精选池替换监控池.

    body: {top_n?: 5, strategy?: str}
      - 不传 strategy / 传空 / "auto" / "match": 双段日线回测匹配 (与晨会一致)
      - 传具体策略名: 整池绑同一策略
    """
    payload = payload or {}
    top_n = int(payload.get("top_n") or 5)
    raw = payload.get("strategy", None)
    if raw is None:
        strategy = None
    else:
        s = str(raw).strip()
        strategy = None if (not s or s.lower() in ("auto", "match")) else s
    try:
        from lib.radar_pool import load_elite_pool
        from lib.live_simulator import LiveSimRunner
        pool = load_elite_pool(top_n=top_n)
        codes = pool.get("codes") or []
        if not codes:
            return {"ok": False, "message": "精选池代码为空", "pool": pool}
        msg = LiveSimRunner().adopt_morning_watch(codes, strategy=strategy)
        return {
            "ok": True,
            "message": msg,
            "scan_date": pool.get("scan_date"),
            "path": pool.get("path"),
            "codes": codes,
            "strategy": strategy or "match",
        }
    except Exception as e:
...
_REAL_NAV_RECORDED_TODAY: str = ""  # YYYY-MM-DD, 进程内防抖


def _maybe_record_real_nav(asset: dict) -> None:
    """实盘视图刷新成功时, 每个交易日最多写一条 NAV 到 real_pnl_history.json."""
    global _REAL_NAV_RECORDED_TODAY
    total = float((asset or {}).get("total_asset") or 0)
    if total <= 0:
        return
    today = datetime.now().strftime("%Y-%m-%d")
    if _REAL_NAV_RECORDED_TODAY == today:
        return
    try:
        from routes.review import _record_real_pnl_snapshot
        r = _record_real_pnl_snapshot(force=False)
        if r.get("ok"):
            _REAL_NAV_RECORDED_TODAY = today
    except Exception as e:
        print(f"[live] 自动记录实盘 NAV 失败: {type(e).__name__}: {e}", flush=True)
...
def real_account():
    """实盘视图用: 拉真实账户 + 持仓 + 当日委托 (5 秒进程级缓存).

    BROKER_MODE=file_bridge 时读大 QMT 策略写入的 inbox/account_snapshot.json；
    否则走 miniQMT / xtquant。
    ...
    from live_trading.qmt_file_bridge import broker_mode, read_account_snapshot
    ...
    if _REAL_CACHE.get("ts") and age < _REAL_CACHE_TTL:
        return {
            ...
            "source":    _REAL_CACHE.get("source") or broker_mode(),
            ...
            "snapshot_ts": _REAL_CACHE.get("snapshot_ts") or "",
            "account_id": _REAL_CACHE.get("account_id") or "",
        }

    if broker_mode() == "file_bridge":
        # 策略默认 10s 写一次；留余量避免短暂卡顿误判断开
        max_age = float(os.environ.get("QMT_SNAPSHOT_MAX_AGE_SEC") or 90)
        snap = read_account_snapshot(max_age_sec=max_age)
        _REAL_CACHE.update({
            ...
            "source": "file_bridge",
            "snapshot_ts": snap.get("ts") or "",
            "account_id": snap.get("account_id") or "",
        })
        # 每个交易日自动写一条实盘 NAV (供复盘生命周期 KPI; 同日跳过)
        _maybe_record_real_nav(snap.get("asset") or {})
        return {
            ...
            "positions": snap.get("positions") or [],
            "orders": snap.get("orders") or [],
            "cached_age_sec": float(snap.get("age_sec") or 0),
            "snapshot_ts": snap.get("ts") or "",
            "account_id": snap.get("account_id") or "",
        }
    ...
    try:
        ...
        _REAL_CACHE.update({"asset": asset, "positions": positions,
                            ...
                            "source": "xtquant"})
        _maybe_record_real_nav(asset)
        return {
            "connected": True, "error": None, "source": "xtquant",
        ...
        _REAL_CACHE.update({"asset": {}, "positions": [], "orders": [],
                            "ts": now, "error": err, "source": "xtquant"})
        return {
            "connected": False, "error": err, "source": "xtquant",
...
def real_order_cancel(payload: Optional[Dict[str, Any]] = Body(None)):
    ...
    body: {"order_id": <合同号>, "code": <可选>}
    file_bridge: 写 outbox 撤单请求，由大 QMT 策略 cancel()；
    xtquant: 直连 MiniQMT 撤单。
    """
    from live_trading.qmt_file_bridge import broker_mode, write_outbox_cancel

    payload = payload or {}
    raw_id = payload.get("order_id")
    if raw_id is None or str(raw_id).strip() == "":
        return {"ok": False, "message": "缺少 order_id（合同号）"}

    if broker_mode() == "file_bridge":
        try:
            req = write_outbox_cancel(
                order_sys_id=str(raw_id).strip(),
                code=str(payload.get("code") or ""),
            )
        except Exception as e:
            return {"ok": False, "message": f"{type(e).__name__}: {e}"}
        # 下次 real_account 重新读快照
        _REAL_CACHE["ts"] = 0.0
        return {
            "ok": True,
            "message": f"已写入撤单文件，等待大 QMT 执行: 合同号 {raw_id}",
...
#   - 实盘视图把这些信号当作 "AI 建议", 用户点 [授权] 才真正委托.
#   - BROKER_MODE=file_bridge 时写大 QMT 文件桥 outbox；xtquant 模式走 MiniQMTTraderV2.
...
_DONE_STATUSES = frozenset({"approved", "rejected", "queued"})
...
def _calc_suggested_quantity(price: float, capital: float, code: str = "") -> int:
    """Kris 建议仓位: min(总资金 10%, ATR 仓位), 100 股一手, 不足按 100 试探"""
    from lib.kris_adapter import suggest_quantity
    if code:
        return suggest_quantity(code, float(price or 0), float(capital or 0))
    if price <= 0:
        return 100
...
def approvals_list():
    ...
    返回:
      {
        "items": [
          {"id", "ts", "code", "side", "strategy", "reason",
           ...
           "status": "pending"|"queued"|"approved"|"rejected"|"expired",
        ...
        "ttl_sec": 300,
        "capital": <float>,
        "broker_mode": "file_bridge"|"xtquant"|...,
      }
    """
    from live_trading.qmt_file_bridge import apply_inbox_to_approvals, broker_mode
    ...
    # 文件桥: 合并大 QMT 回执
    if broker_mode() == "file_bridge":
        approvals, n = apply_inbox_to_approvals(approvals)
        if n:
    ...
    real_positions_map = {}
    real_cash = 0.0
    file_bridge = broker_mode() == "file_bridge"
    if _REAL_CACHE.get("ts") and _REAL_CACHE.get("error") is None:
    ...
    def _eligibility(side: str, code: str, qty: int, price_hint: float):
        ...
        if not _REAL_CACHE.get("ts") or _REAL_CACHE.get("error"):
            # 尚未拉到有效快照/账户，不阻断（点授权时再校验）
            return True, ""
        if side == "sell":
    ...
    for sig in reversed(recent):
        ...
        # 5 分钟未处理 -> 过期; 已处理保持原状态不会被覆盖
        if status == "pending" and age > _APPROVAL_TTL_SEC:
            status = "expired"
        price = pos_price.get(sig.get("code"), 0.0)
        qty = _calc_suggested_quantity(price, capital, code=str(sig.get("code") or ""))
...
def approvals_approve(payload: Optional[Dict[str, Any]] = Body(None)):
    """授权下单: file_bridge 写大 QMT outbox；xtquant 模式走 MiniQMTTraderV2.

    body: {"id": "<signal_id>", "quantity": <int 可选, 覆盖建议数量>, "price": <float 可选, 0=市价>}
    """
    from live_trading.qmt_file_bridge import broker_mode, write_outbox_order
    ...
    approvals = _load_approvals()
    if approvals.get(sid, {}).get("status") in _DONE_STATUSES:
    ...
    # 数量: 用户传了就用, 否则按 Kris 建议仓位算
    state = _load_state()
    capital = float(state.get("capital") or load_mock_config().get("capital") or 1_000_000)
    pos_price = {p.get("code"): float(p.get("cur_price") or 0)
                 for p in (state.get("positions") or [])}
    price_hint = pos_price.get(code, 0.0)
    quantity = int(payload.get("quantity") or _calc_suggested_quantity(price_hint, capital, code=code))
    price = float(payload.get("price") or 0)   # 0 -> 市价

    # Kris 审批参考价: 显式委托价优先, 否则持仓现价 / 最新价
    kris_price = price if price > 0 else float(price_hint or 0)
    if kris_price <= 0:
        try:
            from public_market import get_latest_price, to_std_code
            kris_price = float((get_latest_price(to_std_code(code)) or {}).get("lastPrice") or 0)
        except Exception:
            kris_price = 0.0
    if kris_price <= 0:
        return {"ok": False, "message": f"无法取得 {code} 现价, Kris 无法审批"}

    from lib.kris_adapter import approve_order, decision_to_dict
    decision, quantity, _pf = approve_order(
        code=code, side=side, quantity=quantity, price=kris_price, capital=capital,
    )
    if not decision.is_approved:
        approvals[sid] = {
            ...
            "order": None,
            "error": f"Kris 否决: {decision.reason}",
            "kris": decision_to_dict(decision),
        }
        _save_approvals(approvals)
        return {
            "ok": False,
            "message": f"Kris 否决: {decision.reason}",
            "kris": decision_to_dict(decision),
        }
    kris_verdict = decision_to_dict(decision)

    mode = broker_mode()

    # === 实盘前置校验（有有效账户缓存时，file_bridge / xtquant 都做） ===
    real_acc = _REAL_CACHE if (_REAL_CACHE.get("ts") and _REAL_CACHE.get("error") is None) else None
    if real_acc is not None:
        if side == "sell":
            positions = real_acc.get("positions") or []
            ...
            cash = float((real_acc.get("asset") or {}).get("cash") or 0)
            if cash > 0 and kris_price > 0 and quantity * kris_price > cash:
                return {"ok": False,
                        "message": f"可用现金不足: 需要 ~{quantity * kris_price:,.0f} 元, 实盘可用 {cash:,.0f} 元"}
        ...
        try:
            ...
            if order_id is None or order_id < 0:
                ...
                order = {"order_id": order_id, "code": code, "side": side,
                         "quantity": quantity, "price": price, "kris": kris_verdict,
                         "strategy": str(sig.get("strategy") or "")}
                status = "approved"
                try:
                    from lib.post_fill import after_real_order_accepted
                    after_real_order_accepted(
                        code=code, side=side, price=kris_price,
                        strategy=str(sig.get("strategy") or ""),
            ...
            status = "rejected"
    approvals[sid] = {
        "status":       status if err is None else "rejected",
        "processed_ts": datetime.now().isoformat(timespec="seconds"),
        "order":        order,
        "error":        err,
    }
    ...
    if status == "queued":
        return {
            "ok": True,
            "message": f"已写入大 QMT 委托文件 {side} {code} {quantity}股, 等待客户端执行",
            "order": order,
            "status": "queued",
        }
