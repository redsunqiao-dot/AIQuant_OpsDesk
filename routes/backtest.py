...
@router.post("/report")
@router.post("/report/")
def backtest_qs_report(payload: Optional[Dict[str, Any]] = Body(None)):
    """跑回测并用 QuantStats 生成 HTML 报告.

    body: 同 /run, 另可选 title
    """
    payload = payload or {}
    code = _normalize_code(str(payload.get("code", "")))
    strategy = str(payload.get("strategy", "")).strip()
    ...
    if not code or not strategy or not start or not end:
        return {"ok": False, "message": "需要 code / strategy / start / end"}
    ...
    try:
        ...
        return {"ok": False, "message": f"回测失败: {type(e).__name__}: {e}"}

    if not result.get("ok", True) and result.get("message"):
        # run_backtest 成功时通常直接带 navs
        pass
    navs = result.get("navs") or []
    title = str(payload.get("title") or f"{code} {strategy} QuantStats")
    try:
        from lib.qs_report import generate_qs_html_from_navs
        fname = f"{code.replace('.', '_')}_{strategy}_{start}_{end}.html"
        info = generate_qs_html_from_navs(navs, title=title, out_name=fname)
    except Exception as e:
        return {
            "ok": False,
            "message": f"QuantStats 报告失败: {type(e).__name__}: {e}",
            "backtest": result,
        }
    return {"ok": True, "report": info, "metrics": result.get("metrics"), "code": code,
            "strategy": strategy}
