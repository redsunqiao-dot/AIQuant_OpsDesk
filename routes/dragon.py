...
GET  /api/dragon/candidates    -- 拉今日龙头候选
                                  ...
                                  query 参数:
                                    source=auto|mysql|mock   数据源选择, 默认 auto
...
from dragon.dragon_picker import (
    filter_dragon_candidates, _build_mock_today_stocks,
)
from dragon.dragon_backtest import (
    load_status_meta,
    load_sector_panel,
    load_daily_panel,
...
def _candidates_from_mysql_latest_day(
    ...
    使用本 CASE 内 dragon/dragon_backtest 中的 SQL 拼装；任一步失败则抛错，由上层回退 xtdata / mock。
...
def _candidates_from_xtdata(*args, **kwargs):
    raise RuntimeError('xtdata 已移除，请使用 source=mysql 或 mock')
...
def candidates(
    ...
    source=auto (默认): 优先 mysql -> 失败回退 mock.
    """
    # 兼容旧参数 use_xtdata=1
    if use_xtdata == 1 and source == "auto":
        source = "xtdata"
    source = (source or "auto").lower()
    if source not in ("auto", "mysql", "mock"):
        source = "auto"
    ...
    # ---------- 2) xtdata 路径 ----------
    if False and not cands and source in ("auto", "xtdata"):
        try:
